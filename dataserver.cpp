...
void process_newchannel(RequestChannel* _channel) {
	...
	RequestChannel * data_channel = RequestChannel::get_channel(ipc_type, new_channel_name, RequestChannel::SERVER_SIDE);
...
int main(int argc, char * argv[]) {
	...
	if (argc == 0) {
	    ipc_type =  RequestChannel::Type::MQ;    
    }
	else{
		string ipc_method(argv[4]);
		try {
	...
	RequestChannel* control_channel = RequestChannel::get_channel(ipc_type, "control", RequestChannel::SERVER_SIDE);
